...
function love.load()
    ...
    visualstate:carrier("position", summoner, Vec2(150, 100))
    ...
    animation_ctrl:frames()
        :map(
            function(frame, id)
                local p = visualstate:carrier("position", id):getValue()
                local f = visualstate:carrier("face", id):getValue()
                local w = visualstate:carrier("wooble", id):getValue() or 0
